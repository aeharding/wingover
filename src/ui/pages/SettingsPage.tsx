...
/**
 * The shell is pure chrome — IonPage, the static IonHeader, and the
 * IonContent — kept mounted through a crash so the outlet's stack
 * transitions keep working; the boundary swaps only the body inside
 * IonContent. The header carries no state, so every hook lives in
 * SettingsBody, protected. See PR #133.
 */
    ...
    <>
      ...
      {!isTauri() && (
        <div className={settings.helperText}>
          Browsers can stop background recording at any time. Your phone running
          the Wingover app is the recorder to trust.
        </div>
      )}
      <IonAlert
