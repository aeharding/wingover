...
- **Native feel.** Ionic-polished UI that respects platform conventions, matching the bar Voyager sets. When a color is authored in `display-p3`, no sRGB fallback is added — every target device is wide-gamut, so P3 support is baseline.
...
Toggleable views, PPG Flyer-style:
